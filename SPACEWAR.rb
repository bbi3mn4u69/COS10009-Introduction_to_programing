...
def move_player(player2)
    ...
    player2.velocity_x *= 0.9
    player2.velocity_y *= 0.9
...
#move star
    @stars.push(Star.new(@star_anim)) if rand(50) == 0
    @stars.each do |star| 
        move_star(star) 
    end
#player 1 get star
...
#if two player crash each other 
    @player_bullets.dup.each do |bullet|
        @player2_bullets.dup.each do |player2bullet|
            distance_2_player = Gosu::distance(@player.x, @player.y, @player2.x, @player2.y)
            if distance_2_player < @player.radius + @player2.radius 
                @player_bullets.delete bullet
                @player2_bullets.delete player2bullet
                initialize_end(:player_crash) 
            end
        end
    end
...
#---------------------------------------ending ----------------------------------------
    def initialize_end(status)
        ...
        when :player_crashh
            @message = 'Game over, you are crashing each other!!'
            @message_two = 'Player 1 score, player 2 score'
        when :off_top1_mul
